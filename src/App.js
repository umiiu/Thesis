...
function App() {
            ...
            try {
              ...
              if (isOwnMessage) {
                ...
                if (msg.selfEncryptedContent && msg.selfIv && msg.selfEncryptedKey) {
                  ...
                  decryptedText = '[Cannot decrypt own message - no self-encrypted copy]';
                }
              } else {
                // TIN NHẮN NGƯỜI KHÁC GỬI - dùng encryptedContent
                decryptedText = await decryptMessage(
  ...
  // Lấy danh sách tin nhắn plain text của chat hiện tại để AI có thể tóm tắt
  const currentPlainMessages = selectedChat
    ? (messages[selectedChat.id] || []).map(m => ({
      sender: m.isOwn ? (user?.name || 'You') : selectedChat.name,
      text: m.text
    }))
    : [];
  ...
  return (
    <div className="app-container">
      ...
      {/* ===== AI ASSISTANT (Ollama) ===== */}
      <AIAssistant
        currentMessages={currentPlainMessages}
        selectedChat={selectedChat}
      />
    </div>
  );
